...
public class RestaurantApprovalResponseKafkaListener implements KafkaConsumer<RestaurantApprovalResponseAvroModel> {
	...
	public void receive(@Payload List<RestaurantApprovalResponseAvroModel> messages,
		...
		messages.forEach(message -> {
			try {
				RestrauntApprovalResponse restrauntApprovalResponse = orderMessagingMapper
						.approvalResponseAvroModelToApprovalResponse(message);
				if (OrderApprovalStatus.APPROVED == message.getOrderApprovalStatus()) {
					...
					restrauntApprovalEventListener.orderApproved(restrauntApprovalResponse);
				} else if (OrderApprovalStatus.REJECTED == message.getOrderApprovalStatus()) {
					log.info("Processing rejected order for order id: {}, with failure messages: {}",
							message.getOrderId(), message.getFailureMessages());
					restrauntApprovalEventListener.orderRejected(restrauntApprovalResponse);
				}
